...
def dashboard(db:Session=Depends(database.get_db),admin=Depends(verify_admin)):
...
def update_complaint(ticket_id:int,c_Data:schemas.Complaint_update,admin=Depends(verify_admin),db:Session=Depends(database.get_db)):
    comp=db.get(models.Complaint,ticket_id)
    if comp==None:
        raise Content_Not_Found("Invalid Request")
    elif comp.status.upper()=="APPROVED":
        return Content_Not_Found("Already Aprroved")
...
def update_epasses(ticket_id:int,e_data:schemas.Epass_update,admin=Depends(verify_admin),db:Session=Depends(database.get_db)):
